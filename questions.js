  ...
  // ============================================================
  {
    ...
    questions: [
      ...
      },
      {
        ...
        correct: 0,
        explanation: "3NF vermeidet problematische transitive Abhängigkeiten: Wenn KundenID die AbteilungsID bestimmt und AbteilungsID den Abteilungsnamen, gehört der Abteilungsname in eine eigene Abteilungstabelle. Maßgeblich sind die tatsächlich geltenden fachlichen Abhängigkeiten; PLZ und Ort sind nicht grundsätzlich eindeutig zugeordnet."
      },
      {
        type: "mc",
...
const QUESTION_POOL_MATERIAL_EXTENSION = {
  ...
  "windows-netzwerke": [
    ...
    mc("win-net-050", "gpo", "grundlagen", "Eine GPO wurde erstellt und konfiguriert, aber noch nirgendwo verknüpft. Warum verteilt sie ihre Einstellungen nicht allein dadurch an alle Domänencomputer?", ["GPO-Objekt und Verknüpfung sind getrennt; erst passende Links und Filter bestimmen ihren Wirkungsbereich.", "GPOs benötigen grundsätzlich einen eigenen physischen Server pro Richtlinie.", "Eine GPO wirkt ausschließlich über einen DHCP-Lease.", "Jede neue GPO muss erst in eine NTFS-Berechtigung umgewandelt werden."], 0, "Eine erstellte GPO ist nicht automatisch überall angewendet. Verknüpfungen an Standort, Domäne oder OU und die jeweiligen Filter bestimmen ihre Wirkung. Eine GPO kann mehrere Links haben."),
...
const QUESTION_POOL_SERVER_HYPERV_EXTENSION = {
  "windows-server-admin": [
    ...
    mc("srv-hv-004", "hyper-v-checkpoints", "grundlagen", "Welcher Unterschied zwischen Standard- und Produktionscheckpoint ist fachlich richtig?", ["Ein Produktionscheckpoint erzeugt einen anwendungs- beziehungsweise dateisystemkonsistenten Zustand; ein Standardcheckpoint speichert zusätzlich den laufenden Arbeitsspeicherzustand.", "Ein Standardcheckpoint ist ein unabhängiges Offline-Backup, ein Produktionscheckpoint nicht.", "Produktionscheckpoints funktionieren nur bei ausgeschalteten VMs.", "Beide Verfahren sind technisch identisch und unterscheiden sich nur im Namen."], 0, "Produktionscheckpoints nutzen Mechanismen des Gastsystems für einen konsistenten Zustand. Standardcheckpoints speichern den vollständigen VM-Zustand einschließlich Arbeitsspeicher und eignen sich eher für Testumgebungen."),
    ...
    mc("srv-hv-015", "failover-cluster", "fortgeschritten", "Welches Problem löst das Quorum eines Failover-Clusters hauptsächlich?", ["Es verhindert, dass getrennte Clusterteile gleichzeitig widersprüchliche Entscheidungen treffen.", "Es verschlüsselt automatisch alle VHDX-Dateien.", "Es ersetzt DNS für alle Clusterrollen.", "Es verteilt Benutzerkonten auf Organisationseinheiten."], 0, "Quorum stellt sicher, dass nur ein entscheidungsfähiger Clusterteil Rollen verwaltet. Dadurch wird ein Split-Brain-Betrieb vermieden."),
    ...
    mc("srv-hv-019", "failover-cluster", "fortgeschritten", "Warum ist ein Failover-Cluster kein Ersatz für ein Backup?", ["Der Cluster kann logische Fehler, versehentliches Löschen oder Ransomware auf den gemeinsam genutzten Daten nicht zuverlässig rückgängig machen.", "Ein Cluster darf grundsätzlich keine virtuellen Maschinen ausführen.", "Backups funktionieren ausschließlich auf Einzelservern.", "Ein Cluster besitzt keine Netzwerkverbindungen."], 0, "Hochverfügbarkeit reduziert Ausfallzeiten bestimmter Komponenten. Ein Backup stellt frühere, unabhängige Datenstände wieder her und schützt damit vor anderen Schadensarten."),
    ...
    mc("srv-hv-023", "hyper-v-netzwerk", "fortgeschritten", "Welche Wirkung wird mit RDMA bei Hyper-V-Storage- oder Migrationsverkehr hauptsächlich angestrebt?", ["Hoher Datendurchsatz bei geringer CPU-Belastung und niedriger Latenz", "Automatische Verschlüsselung jeder VM", "Ersetzen aller IP-Routen durch MAC-Adressen", "Umwandlung eines privaten Switches in einen externen Switch"], 0, "RDMA ermöglicht Netzwerkkarten einen weitgehend direkten Datentransfer in den Arbeitsspeicher und entlastet dadurch die CPU."),
    mc("srv-hv-024", "hyper-v-netzwerk", "anspruchsvoll", "Ein Unternehmen setzt RoCE für Storage-Verkehr ein. Welche zusätzliche Planung ist besonders wichtig?", ["DCB und gegebenenfalls PFC müssen über den gesamten beteiligten Ethernet-Pfad konsistent konfiguriert werden.", "Alle Switches müssen DNS-Server werden.", "Jede VM muss einen privaten Hyper-V-Switch verwenden.", "RoCE funktioniert nur über eine analoge Telefonleitung."], 0, "RoCE reagiert empfindlich auf Paketverlust. Deshalb muss die verlustarme Ethernet-Konfiguration mit DCB/PFC Ende zu Ende geplant und geprüft werden."),
    ...
    mc("srv-hv-030", "hyper-v-kompatibilitaet", "anspruchsvoll", "Nach dem Aktualisieren der Konfigurationsversion einer VM kann ein älterer Hyper-V-Host sie nicht mehr übernehmen. Was ist die fachlich passende Erklärung?", ["Der ältere Host unterstützt die neuere VM-Konfigurationsversion nicht; ein Upgrade sollte daher erst nach Prüfung aller Zielhosts erfolgen.", "Die VM hat automatisch ihre IP-Adresse verloren.", "Eine Konfigurationsversion betrifft nur die Farbe des Hyper-V-Managers.", "Jede neuere Konfigurationsversion ist vollständig abwärtskompatibel."], 0, "Die Konfigurationsversion bestimmt, welche Hyper-V-Funktionen und Hosts die VM unterstützen. Ein Update kann die Rückkehr auf ältere Hosts verhindern."),
    mc("srv-hv-031", "hyper-v-kompatibilitaet", "anspruchsvoll", "Wozu dient der Prozessor-Kompatibilitätsmodus einer Hyper-V-VM hauptsächlich?", ["Er blendet bestimmte neuere CPU-Funktionen aus, damit eine Migration zwischen unterschiedlichen Prozessorgenerationen desselben Herstellers eher möglich wird.", "Er erhöht automatisch die Zahl physischer CPU-Kerne.", "Er macht Intel- und AMD-Prozessoren grundsätzlich austauschbar.", "Er ersetzt die Prüfung der VM-Konfigurationsversion."], 0, "Der Modus stellt der VM einen gemeinsamen, eingeschränkten Funktionsumfang bereit. Er ist kein allgemeiner Ersatz für Hostkompatibilität und macht unterschiedliche CPU-Hersteller nicht beliebig austauschbar."),
    ...
    mc("srv-ops-003", "baseline", "anspruchsvoll", "Ein Server weicht nach mehreren ungeplanten Änderungen von der dokumentierten Sicherheitsbaseline ab. Warum ist das problematisch?", ["Die Abweichungen können Schutz, Supportfähigkeit und Fehlersuche beeinträchtigen und sind ohne nachvollziehbare Dokumentation schwer zu bewerten.", "Eine Baseline darf nach der Installation grundsätzlich nie angepasst werden.", "Jede Abweichung verbessert automatisch die Sicherheit.", "Baselines gelten nur für Desktop-Hintergründe."], 0, "Eine Baseline ist ein kontrollierter Ausgangszustand. Notwendige Abweichungen müssen begründet, getestet, freigegeben und dokumentiert werden."),
    mc("srv-ops-004", "bereitstellung", "fortgeschritten", "Die DNS-Serverrolle ist installiert, aber Clients können keine Namen auflösen. Welche Aussage ist fachlich richtig?", ["Die Rolleninstallation stellt nur Komponenten bereit; Zonen, Records, Netzwerk, Firewall, Dienststatus und Funktion müssen anschließend konfiguriert und geprüft werden.", "Eine installierte Rolle ist immer automatisch vollständig produktionsbereit.", "DNS funktioniert erst nach der Installation von Hyper-V.", "Der Server muss lediglich in Desktop Experience umgewandelt werden."], 0, "Rollenbereitstellung und Rollenfunktion sind getrennte Schritte. Nach Installation folgen Konfiguration, Abhängigkeitsprüfung, Absicherung und ein fachlicher Funktionstest.")
  ],
  ...
  "netzwerke": [
    ...
    mc("net-ipv6-005", "ipv6-neighbor-discovery", "fortgeschritten", "Welche Aufgabe übernimmt Neighbor Discovery bei IPv6 unter anderem?", ["Es ermittelt Nachbarn und Router und übernimmt Funktionen, die bei IPv4 unter anderem ARP erfüllt.", "Es verschlüsselt automatisch alle TCP-Verbindungen.", "Es ersetzt das Routing zwischen entfernten Netzen.", "Es vergibt Windows-Dateiberechtigungen."], 0, "Neighbor Discovery verwendet ICMPv6 für Nachbarerkennung, Routererkennung und weitere Kontrollfunktionen. ARP wird bei IPv6 nicht verwendet."),
...
const QUESTION_POOL_MASTER_EXTENSION = {
  ...
  "netzwerke": [
    ...
    mc("master-net-03", "dhcp-relay", "fortgeschritten", "Clients in VLAN 20 sollen einen DHCPv4-Server in VLAN 10 nutzen. Routing ist vorhanden, die ersten DHCP-Broadcasts erreichen den Server jedoch nicht. Was wird zusätzlich benötigt?", ["Eine DNS-Reverse-Lookup-Zone für VLAN 20", "Eine identische statische IP-Adresse für alle Clients", "Ein DHCP-Relay im Clientnetz mit passendem DHCP-Bereich für VLAN 20", "Ein längeres DNS-Cache-Intervall"], 2, "Ein Relay nimmt DHCP-Nachrichten im Clientnetz entgegen und leitet sie an den Server weiter. Der Server benötigt einen passenden Bereich und einen funktionierenden Rückweg; normales Routing allein leitet den Broadcast nicht weiter."),
    ...
    mc("master-net-07", "dhcp-failover", "anspruchsvoll", "Welchen Zweck erfüllt die Maximum Client Lead Time (MCLT) bei DHCP-Failover?", ["Sie legt die Lebensdauer aller DNS-Records fest.", "Sie begrenzt den zeitlichen Vorsprung von Leaseverlängerungen gegenüber dem Partnerwissen und schützt die koordinierte Übernahme.", "Sie ist die maximale Laufzeit eines Ethernet-Frames.", "Sie ersetzt die Authentisierung zwischen den Failoverpartnern."], 1, "MCLT begrenzt, wie weit eine Lease über den dem Partner bekannten Stand hinaus verlängert werden darf. Sie ist außerdem bei der sicheren Übernahme von Adressen relevant und nicht einfach die normale Client-Leasezeit."),
    ...
    mc("master-net-10", "dns-aufloesung", "fortgeschritten", "Was unterscheidet eine rekursive von einer iterativen DNS-Anfrage?", ["Bei Rekursion soll der angesprochene Resolver die Auflösung erledigen; bei Iteration kann eine Antwort auf einen anderen Nameserver verweisen.", "Rekursion betrifft nur IPv6, Iteration nur IPv4.", "Iterative Anfragen sind grundsätzlich verschlüsselt, rekursive nie.", "Rekursion liefert ausschließlich PTR-Records, Iteration ausschließlich A-Records."], 0, "Ein Client erwartet vom rekursiven Resolver eine abschließende Antwort oder einen Fehler. Der Resolver kann bei der weiteren Suche iterative Anfragen verwenden und Verweisen folgen."),
    ...
    mc("master-net-13", "dns-betrieb", "anspruchsvoll", "In einer dynamisch aktualisierten DNS-Zone bleiben Records längst entfernter Clients zurück. Welcher Mechanismus ist dafür vorgesehen, solche veralteten Einträge nach passenden Fristen zu bereinigen?", ["Die Verlängerung aller DHCP-Leases", "DNS-Aging und Scavenging mit sorgfältig geplanten Zeitintervallen", "Die Umwandlung jedes A-Records in einen CNAME", "Das dauerhafte Abschalten des DNS-Caches aller Clients"], 1, "Aging erfasst die Alterung dynamischer Records; Scavenging kann ausreichend veraltete Einträge entfernen. Zeitstempel, Aktualisierung und Fristen müssen stimmen, damit gültige Einträge erhalten bleiben."),
  ...
  "it-grundlagen": [
    ...
    mc("master-base-03", "speicherschnittstellen", "grundlagen", "Warum ist die Aussage 'Jede M.2-SSD verwendet NVMe' falsch?", ["NVMe bezeichnet ausschließlich eine mechanische Festplatte.", "M.2 ist ein Dateisystem und legt deshalb kein Protokoll fest.", "M.2 beschreibt einen Formfaktor; je nach Gerät und Anschluss sind beispielsweise SATA oder PCIe/NVMe möglich.", "M.2 und NVMe bezeichnen beide ausschließlich die Speicherkapazität."], 2, "Formfaktor, Schnittstelle und Protokoll sind verschiedene Eigenschaften. Eine M.2-SSD muss elektrisch und protokollseitig zum Steckplatz passen; die Bauform allein genügt nicht."),
    ...
    mc("master-base-07", "cloud-api", "fortgeschritten", "Ein Verwaltungsskript soll freigegebene Microsoft-365-Benutzerdaten über eine API abrufen. Welche Rolle hat Microsoft Graph dabei?", ["Es stellt einen API-Zugang bereit, dessen Datenzugriff passende Berechtigungen und Authentisierung benötigt.", "Es ersetzt die Authentisierung durch die Kenntnis einer beliebigen E-Mail-Adresse.", "Es verschiebt automatisch alle lokalen Domänencontroller in Azure.", "Es ist ausschließlich ein Dateiformat für Diagrammbilder."], 0, "Microsoft Graph erschließt Microsoft-365- und Entra-Daten über APIs. Authentisierung, Einwilligung beziehungsweise Berechtigungen und minimale Rechte bleiben notwendig; JSON ist ein Datenformat und keine Zugriffsberechtigung."),
    mc("master-base-08", "cloud-automatisierung", "grundlagen", "Was beschreibt das Grundprinzip eines ereignisgesteuerten Power-Automate-Flows?", ["Ein Datenträger wird bei jedem Start neu partitioniert.", "Ein Auslöser startet definierte Aktionen, gegebenenfalls mit Bedingungen und Daten aus verbundenen Diensten.", "Ein DNS-Server wandelt alle Tabellen automatisch in JSON um.", "Ein Hypervisor verteilt seine CPU-Kerne ausschließlich anhand von E-Mails."], 1, "Ein Trigger, etwa das Eintreffen eines Formulars, startet Aktionen wie eine Genehmigungsanforderung. Verbindungen und Berechtigungen müssen passend eingerichtet sein."),
    mc("master-base-09", "cloud-datenschutz", "fortgeschritten", "Ein Team synchronisiert seinen Projektordner mit OneDrive. Eine versehentliche Löschung wird ebenfalls synchronisiert. Welche Schlussfolgerung ist richtig?", ["Synchronisierung bewahrt zwangsläufig jeden jemals gespeicherten Stand unbegrenzt auf.", "Synchronisierte Dateien können grundsätzlich nicht wiederhergestellt werden.", "Synchronisierung allein ist kein vollständiges Backupkonzept; Versionierung, Aufbewahrung und Wiederherstellungswege müssen geprüft werden.", "Die Löschung zeigt, dass OneDrive nur ein lokales Dateisystem ohne Cloudfunktion ist."], 2, "Synchronisierung verteilt auch unerwünschte Änderungen. Vorhandene Versionen und Papierkörbe können helfen, ersetzen aber keine Prüfung von Aufbewahrungsfristen, Risiken und Wiederherstellung."),
  ...
  "it-sicherheit": [
    ...
    mc("master-sec-05", "mobile-geraete", "fortgeschritten", "Ein verwaltetes Smartphone geht verloren und ist ausgeschaltet. Warum darf sich die IT nicht allein auf eine sofortige MDM-Fernlöschung verlassen?", ["MDM kann grundsätzlich keine Geräte verwalten, die den Standort verlassen.", "Eine Fernlöschung ist gleichbedeutend mit einer bereits abgeschlossenen Datensicherung.", "Die Ausführung hängt von Registrierung, Plattform und Erreichbarkeit ab; Gerätesperre und Verschlüsselung müssen bereits vorher wirken.", "Das Ausschalten entfernt automatisch alle Unternehmensdaten."], 2, "Ein Offlinegerät kann den Löschbefehl zunächst nicht empfangen. Vorbeugender Schutz, Sperrung von Zugängen und ein geregelter Vorfallprozess sind zusätzlich erforderlich."),
  ...
  "windows-netzwerke": [
    mc("master-win-01", "sid", "fortgeschritten", "Ein AD-Benutzer wird gelöscht und später mit demselben Anmeldenamen neu angelegt. Warum greifen seine alten, direkt vergebenen Dateirechte nicht automatisch wieder?", ["Das neue Konto erhält eine neue SID, während die alten Berechtigungseinträge die frühere SID referenzieren.", "Der Anmeldename wird von NTFS grundsätzlich nicht unterstützt.", "Nach jeder Anmeldung ändern sich alle SIDs automatisch.", "Das neue Konto erhält immer die SID des eingebauten Administrators."], 0, "Berechtigungen beziehen sich auf Sicherheitskennungen, nicht nur auf sichtbare Namen. Umbenennen erhält die SID; Löschen und Neuanlegen erzeugt dagegen eine neue Identität. Der RID ist der relative, letzte Teil einer entsprechenden SID."),
    ...
    mc("master-win-07", "gpo-kennwoerter", "anspruchsvoll", "Nur eine bestimmte Gruppe von Domänenbenutzern soll abweichende Kennwort- und Kontosperrvorgaben erhalten. Welches AD-Konzept ist dafür vorgesehen?", ["Eine andere Bildschirm-GPO an der Benutzer-OU", "Ein abweichender DNS-Suffix für die Gruppe", "Fine-Grained Password Policies über Password Settings Objects für Benutzer oder globale Sicherheitsgruppen", "Ein zusätzlicher DHCP-Bereich mit längerer Leasezeit"], 2, "PSOs ermöglichen differenzierte Kennwort- und Sperrrichtlinien. Sie werden nicht direkt einer OU zugewiesen; eine beliebige OU-Kennwort-GPO ersetzt dieses Konzept für Domänenkonten nicht."),
    ...
    mc("master-win-09", "gpo-preferences", "fortgeschritten", "Eine Einstellung wurde per Group Policy Preferences gesetzt. Der Computer verlässt später den Wirkungsbereich. Warum darf man nicht pauschal erwarten, dass der frühere lokale Wert wiederhergestellt wird?", ["Preferences können Änderungen hinterlassen; die Rücknahme hängt unter anderem von Einstellung und Entfernungskonfiguration ab.", "Preferences werden grundsätzlich niemals auf Clients ausgeführt.", "Jede Preference erzeugt automatisch ein vollständiges Clientbackup.", "Der Wirkungsbereich gilt nur für DNS-Server und nicht für Computer."], 0, "Preferences setzen bevorzugte Konfigurationen und können Werte dauerhaft verändern. Richtlinien und Preferences haben unterschiedliche Verhaltensweisen; eine automatische Rücknahme muss gezielt geprüft werden."),
    mc("master-win-10", "gpo-aktualisierung", "fortgeschritten", "Eine korrekt verknüpfte GPO wurde geändert. Was bewirkt gpupdate /force auf einem erreichbaren Windows-Client?", ["Es verschiebt das Computerkonto automatisch in die richtige OU.", "Es stößt eine erneute Verarbeitung der Benutzer- und Computerrichtlinien an; manche Änderungen brauchen weiterhin Abmeldung oder Neustart.", "Es ersetzt alle fehlenden Leserechte auf die GPO.", "Es ändert den Standard-Aktualisierungsabstand dauerhaft auf eine Sekunde."], 1, "Die normale Hintergrundaktualisierung liegt bei Domänenclients typischerweise bei 90 Minuten plus bis zu 30 Minuten Zufallsversatz, bei DCs bei fünf Minuten. gpupdate repariert keine falsche Verknüpfung oder Filterung."),
    mc("master-win-11", "benutzerprofile", "grundlagen", "Was unterscheidet ein servergespeichertes Benutzerprofil von einer Ordnerumleitung?", ["Ein servergespeichertes Profil kopiert zwingend alle installierten Programme auf jeden PC.", "Ordnerumleitung verschiebt automatisch die gesamte Windows-Installation auf den Server.", "Ein servergespeichertes Profil synchronisiert unterstützte Profilbestandteile; Ordnerumleitung lenkt bestimmte Benutzerordner auf einen anderen Speicherort.", "Beide Begriffe bezeichnen die Replikation der AD-Datenbank zwischen DCs."], 2, "Profile betreffen Benutzereinstellungen und Profildaten, Ordnerumleitung ausgewählte Ordner wie Dokumente. Installierte Anwendungen folgen nicht automatisch; Profilversionen, Rechte und Offlineverhalten müssen berücksichtigt werden.")
  ],
  ...
  "windows-server-admin": [
    ...
    mc("master-srv-04", "storage-spaces", "fortgeschritten", "Ein Storage-Pool enthält 10 TB physischen Speicher. Per Thin Provisioning wurden Volumes mit zusammen 18 TB logischer Kapazität angelegt. Was muss der Betrieb besonders überwachen?", ["Ob der tatsächlich belegte physische Speicher rechtzeitig erweitert werden muss", "Ob jedes Volume denselben Laufwerksbuchstaben besitzt", "Ob die logische Kapazität automatisch zusätzliche physische Laufwerke erzeugt", "Ob alle gespeicherten Dateien einen identischen Namen haben"], 0, "Thin Provisioning reserviert physischen Platz nach Bedarf, nicht vollständig bei der logischen Zuweisung. Ohne rechtzeitige Erweiterung kann der Pool voll werden, obwohl ein Volume noch freie logische Kapazität anzeigt."),
    ...
    mc("master-srv-07", "ntfs-vererbung", "fortgeschritten", "Eine Datei wird im Windows-Explorer innerhalb desselben NTFS-Volumes in einen anderen Ordner verschoben. Es gibt keine Sonderkonfiguration oder nachträgliche ACL-Anpassung. Was geschieht normalerweise mit ihren bisherigen NTFS-Berechtigungen?", ["Sie werden immer vollständig durch Vollzugriff für Jeder ersetzt.", "Sie bleiben beim Verschieben innerhalb desselben Volumes grundsätzlich erhalten.", "Sie werden in Freigabeberechtigungen umgewandelt.", "Die Datei verliert alle Berechtigungseinträge und wird automatisch gelöscht."], 1, "Beim üblichen Verschieben innerhalb desselben NTFS-Volumes bleibt das Objekt mit seinen Rechten erhalten. Kopieren oder volumenübergreifendes Verschieben erzeugt dagegen typischerweise ein neues Objekt mit Zielvererbung. Werkzeuge mit expliziten Sicherheitsoptionen können davon abweichen."),
    ...
    mc("master-srv-13", "hyper-v-ressourcen", "fortgeschritten", "Ein Host besitzt acht physische CPU-Kerne. Seinen VMs werden insgesamt 24 vCPUs zugewiesen. Welche Aussage beschreibt diese Zuweisung korrekt?", ["Jede vCPU ist ein zusätzlicher physischer Kern, sodass nun 32 Kerne vorhanden sind.", "Virtuelle Prozessoren teilen sich die reale CPU-Zeit; die tatsächliche Leistung hängt von Last und Planung ab.", "Die Zuweisung garantiert jeder VM jederzeit 24 dedizierte Kerne.", "CPU-Überbuchung lässt sich allein durch eine größere VHDX-Datei ausgleichen."], 1, "vCPUs sind virtuelle Ausführungseinheiten, keine neu entstandenen physischen Kerne. Überbuchung kann sinnvoll sein, benötigt aber Kapazitätsplanung und Messung der tatsächlichen Last."),
    mc("master-srv-14", "hyper-v-arbeitsspeicher", "fortgeschritten", "Mehrere Hyper-V-VMs verwenden dynamischen Arbeitsspeicher. Was ist bei der Planung des Host-RAMs trotzdem erforderlich?", ["Es genügt, für jede VM beliebige Maximalwerte einzutragen; physischer RAM ist dann unbegrenzt.", "Der Host darf seinen gesamten Arbeitsspeicher ohne Reserve an Gäste binden.", "Dynamischer Arbeitsspeicher reserviert automatisch für jede VM dauerhaft deren Maximalwert.", "Realen Gesamtbedarf und Hostreserve einplanen; dynamische Zuteilung erzeugt keinen zusätzlichen physischen RAM."], 3, "Dynamischer Arbeitsspeicher kann die Zuteilung innerhalb konfigurierter Grenzen anpassen. Bei gleichzeitiger Last bleibt die physische Kapazität eine Grenze; auch der Host benötigt Ressourcen."),
    mc("master-srv-15", "hyper-v-numa", "fortgeschritten", "Warum kann bei großen VMs die NUMA-Topologie eines Hosts wichtig sein?", ["Der Zugriff auf prozessornahen Speicher kann günstiger sein als auf Speicher eines anderen NUMA-Knotens.", "NUMA ersetzt die Vergabe von IP-Adressen innerhalb der VM.", "NUMA garantiert unabhängig von VM-Größe und Last stets identische Speicherlatenzen.", "NUMA ist ausschließlich ein Verfahren zum Komprimieren von VHDX-Dateien."], 0, "Bei NUMA sind Speicherzugriffswege nicht gleichwertig. Große VMs und ihre vCPU-/RAM-Verteilung sollten deshalb zur Hosttopologie und Arbeitslast passen."),
    mc("master-srv-16", "uac", "grundlagen", "Welche Aussage beschreibt die Benutzerkontensteuerung UAC korrekt?", ["Jede Bestätigung macht einen Standardbenutzer dauerhaft zum Domänenadministrator.", "UAC ersetzt die NTFS-Zugriffsprüfung für alle Programme.", "UAC unterstützt die kontrollierte Ausführung mit erhöhten Rechten; eine Bestätigung erzeugt nicht beliebige neue Berechtigungen.", "UAC ist lediglich ein anderer Name für Windows Update."], 2, "Auch Mitglieder der lokalen Administratoren arbeiten üblicherweise mit getrenntem normalem und erhöhtem Kontext. Standardbenutzer benötigen für administrative Aktionen passende zusätzliche Anmeldeinformationen; UAC schafft keine pauschale Domänenberechtigung.")
  ]
};
